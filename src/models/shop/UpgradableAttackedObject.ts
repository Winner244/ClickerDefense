...
export class UpgradableAttackedObject extends AttackedObject implements IUpgradableObject{
	...
	// parameter ids
	static readonly IMPROVE_HEALTH_PARAMETER: string = 'upgradable_attacked_improve_health_parameter';
	static readonly DEFENSE_PARAMETER: string = 'upgradable_attacked_defense_parameter';
	...
	{
		super(x, y, healthMax, scaleSize, image, isLeftSide, isLand, reduceHover, name, shopName, imageHandler, frames, animationDurationMs);
	...
	loadedResourcesAfterBuild(){
		this.infoItems = [
			new ParameterItem(UpgradableAttackedObject.IMPROVE_HEALTH_PARAMETER, 'Здоровье', this.improveHealthGetValue.bind(this), HealthIcon, 13, () => this.price - this.price / 5, () => this.improveHealth(this.initialHealthMax)),

			new ParameterItem(UpgradableAttackedObject.DEFENSE_PARAMETER, 'Защита', () => this.defense.toFixed(1), ShieldIcon, 13, () => this.price * (this.defense + 1), () => this.defense += 1)
		];
	}
