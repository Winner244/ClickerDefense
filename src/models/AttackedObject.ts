...
export class AttackedObject {
	...
	reduceHover: number; //на сколько пикселей уменьшить зону наведения для игрока? (помимо этого - применяется ещё и проверка пикселей)
	explositionScaleSize: number = 1; //масштаб взрыва в процентах (1 - так и остаётся, 0.5 - в 2 раза меньше)
	...
	constructor(x: number, y: number, healthMax: number, scaleSize: number, image: HTMLImageElement, isLeftSide: boolean, isLand: boolean, reduceHover: number, 
	...
	addModifier(newModifier: Modifier): void{
		...
		if(existedModifier){
			if(existedModifier instanceof FireModifier && newModifier instanceof FireModifier){
				existedModifier.fireDamageInSecondPercentage = Math.max(existedModifier.fireDamageInSecondPercentage || 0, newModifier.fireDamageInSecondPercentage || 0);
	...
	containsPoint(x: number, y: number): boolean { //TODO: use scaleSize also
	...
	drawMofidiersBehindObject(drawsDiffMs: number, isGameOver: boolean){
		this.modifiers.forEach(modifier => modifier.drawBehindObject(this, drawsDiffMs, isGameOver, 0, 0));
	}

	drawMofidiersAheadObject(drawsDiffMs: number, isGameOver: boolean){
		this.modifiers.forEach(modifier => modifier.drawAheadObject(this, drawsDiffMs, isGameOver, 0, 0));
	}
	...
	drawBase(drawsDiffMs: number, isGameOver: boolean, x: number|null = null, y: number|null = null, filter: string|null = null){
		...
		this.drawMofidiersBehindObject(drawsDiffMs, isGameOver);
		...
		this.drawMofidiersAheadObject(drawsDiffMs, isGameOver);
	}

	drawModifiersAhead(drawsDiffMs: number, isGameOver: boolean){
		this.modifiers.forEach(modifier => modifier.drawAheadObjects(this, drawsDiffMs, isGameOver, 0, 0));
	}
		...
		isInvertAnimation: boolean = false)
	{
		...
		if(imageOrAnimation instanceof OffscreenCanvas){ //without filter 
			...
			var frame = imageOrAnimation.draw(drawsDiffMs, isGameOver, invertSign * x, y, invertSign * this.width, this.height, filter, isInvertAnimation);
