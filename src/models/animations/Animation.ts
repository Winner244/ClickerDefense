...
export default class Animation extends AnimationBase {
	...
	draw(drawsDiffMs: number, isGameOver: boolean, x: number, y: number, width: number, height: number, filter: string|null = null, isInvert: boolean = false): number{
		...
		this.currentFrame = this.lastFrame = this.getCurrentFrame(isGameOver, isInvert);
		this.drawBase(this.canvas || Draw.ctx, filter, x, y, width, height);

		return this.currentFrame;
	}

	getCurrentFrame(isGameOver: boolean, isInvert: boolean): number{
