...
import AnimationRandom from '../../models/animations/AnimationRandom';
...
import {NecromancerCharge} from '../movingObjects/NecromancerCharge';
...
export class Necromancer extends Monster{
	...
	attackSimple(damage: number): void { //энергетический шар
		if(damage > 0 && this._goal != null){
			var chargeImage = NecromancerCharge.chargeImage;
			let x1 = this.isLeftSide 
				? this.x + this.width 
				: this.x;
			let y1 = this.centerY - 15;
			let x2 = this._goal.centerX - chargeImage.width / 2;
			let y2 = this._goal.centerY - chargeImage.height / 2;
			...
			var chargeMovingObject = new MovingObject(x1, y1, chargeImage.width, chargeImage.height, 1000 * 20, dx, dy, rotate);
			var charge = new NecromancerCharge(chargeMovingObject, damage);
			MovingObjects.all.push(charge);
	...
	drawObject(drawsDiffMs: number, imageOrAnimation: AnimationInfinite|Animation|HTMLImageElement, isGameOver: boolean, invertSign: number){
		...
		if (this._isDebufStarted){
			...
			var frame = this.debufAnimation.draw(drawsDiffMs, isGameOver, invertSign * this.x + (invertSign < 0 ? 27 * this.scaleSize : 0), this.y - 17 * this.scaleSize, invertSign * newWidth, newHeight);
			this.setCanvas(this.debufAnimation, frame);
		}
			...
			var frame = this.defenseInfinityAnimation.draw(drawsDiffMs, isGameOver, invertSign * this.x + (invertSign < 0 ? 27 * this.scaleSize : 0), this.y - 17 * this.scaleSize, invertSign * newWidth, newHeight);
			this.setCanvas(this.defenseInfinityAnimation, frame);
		}
			...
			var frame = this.defenseCreatingAnimation.draw(drawsDiffMs, isGameOver, invertSign * this.x + (invertSign < 0 ? 27 * this.scaleSize : 0), this.y - 17 * this.scaleSize, invertSign * newWidth, newHeight);
			this.setCanvas(this.defenseCreatingAnimation, frame);
		}
		else if (this._isDefenseEnding){
			const newWidth = this.defenseCreatingAnimation.image.width / this.defenseCreatingAnimation.frames * this.scaleSize;
			const newHeight = this.defenseCreatingAnimation.image.height * this.scaleSize;
			var frame = this.defenseCreatingAnimation.draw(drawsDiffMs, isGameOver, invertSign * this.x + (invertSign < 0 ? 27 * this.scaleSize : 0), this.y - 17 * this.scaleSize, invertSign * newWidth, newHeight, null, true);
			this.setCanvas(this.defenseCreatingAnimation, frame);
		}
			...
			var frame = this.specialAbilityAcidRainCallAnimation.draw(drawsDiffMs, isGameOver, invertSign * this.x + (invertSign < 0 ? 27 * this.scaleSize : 0), this.y - 17 * this.scaleSize, invertSign * newWidth, newHeight);
			this.setCanvas(this.specialAbilityAcidRainCallAnimation, frame);
			...
			if(this._isSpecialAbilityAcidRainCreatingStarted && this._goal && this.specialAbilityAcidRainCreatingAnimation.leftTimeMs > 0){
				...
				var frame = this.specialAbilityAcidRainCreatingAnimation.draw(drawsDiffMs, isGameOver, invertSign * x, y, invertSign * width, height);
				this.setCanvas(this.specialAbilityAcidRainCreatingAnimation, frame);
			}
		}
			...
			var frame = this.specialAbilityCallSkeletesAnimation.draw(drawsDiffMs, isGameOver, invertSign * this.x + (invertSign < 0 ? 27 * this.scaleSize : 0), this.y - 17 * this.scaleSize, invertSign * newWidth, newHeight);
			this.setCanvas(this.specialAbilityCallSkeletesAnimation, frame);
		}
		else{
