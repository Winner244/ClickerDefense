...
export class Monsters{
	...
	static mouseLogic(mouseX: number, mouseY: number, isClick: boolean, isHoverFound: boolean): boolean{
		...
		for(let i = Monsters.all.length - 1; i >= 0; i--){ //в обратном порядке, что бы кликался сперва тот - который виден первым 
			...
			{
				if (monster.containsPoint(mouseX - monster.x, mouseY - monster.y)) 
				{
			...
			for(let i = 0; i < Monsters.all.length; i++){
				...
				if(monster.health <= 0){
					...
					this.explosions.push(new SimpleObject(
						...
						monster.width * monster.explositionScaleSize, //width
						monster.height * monster.explositionScaleSize, //height
						this.explosionAnimation.durationMs));
