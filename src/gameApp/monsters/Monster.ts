...
export class Monster extends AttackedObject{
	...
	protected readonly attackFirstTimeWaitingMs: number; //ожидание перед первым ударом до применения урона (в миллисекундах)
	_attackLeftTimeMs: number; //оставшееся время до следующей атаки (миллисекунды)
	_attackFirstLeftTimeMs: number; //оставшееся время до первой атаки (миллисекунды)
	...
	protected _shiftY: number; //дополнительный сдвиг по оси y (чем больше, тем выше отображается)
	...
	constructor(
		...
		attackFirstTimeWaitingMs: number,
		speed: number,
		...
		avrTimeSoundWaitMs: number)
	{
		...
		this.attackFirstTimeWaitingMs = attackFirstTimeWaitingMs;
		this._attackFirstLeftTimeMs = attackFirstTimeWaitingMs;
	...
	selectGoal(buildings: Building[], monsters: Monster[], units: Unit[]): void{
		if(this._goal == null || this._goal.health <= 0 || this.isLand && units.filter(x => x.isLand).length){
			...
			goals = goals.concat(buildings).concat(units).filter(x => x.isLand == this.isLand && x.health > 0 && (this.isLeftSide ? this.centerX < x.x : this.centerX > x.x + x.width));
	...
	logic(drawsDiffMs: number, buildings: Building[], monsters: Monster[], units: Unit[], bottomBorder: number, waveLevel: WaveData[]): void{
		...
		{
			if(this._attackFirstLeftTimeMs > 0)
				this._attackFirstLeftTimeMs -= drawsDiffMs;
			
			//атака
			if(this._attackLeftTimeMs <= 0 && this._attackFirstLeftTimeMs <= 0){
			...
			//гравитация
			if(this.isLand){
				if(this.y > bottomBorder - this.attackHeight + 2 - this._shiftY){
					this.y-=3;
				}
				else if(this.y < bottomBorder - this.attackHeight - this._shiftY){
					this.y++;
				}
			}
		}
		else {
			this._attackFirstLeftTimeMs = this.attackFirstTimeWaitingMs;

			//гравитация
			if(this.isLand){
				if(this.y > bottomBorder - this.height + 2 - this._shiftY){
					this.y-=3;
				}
				else if(this.y < bottomBorder - this.height - this._shiftY){
					this.y++;
				}
			}
		}
	...
	attack(damage: number): void{
		if(damage > 0 && this._goal != null){
			...
			var volume = Math.min(20, -5 + 1 * Math.sqrt(sizeVolumeScale) * Math.sqrt(Math.sqrt(Math.pow(damage, 3))));
	...
	drawObject(drawsDiffMs: number, imageOrAnimation: AnimationInfinite|Animation|HTMLImageElement, isGameOver: boolean, invertSign: number = 1){
		if(this._isAttack){
			//атака
			var frame = this.attackAnimation.draw(drawsDiffMs, isGameOver, invertSign * this.x, this.y, invertSign * this.attackWidth, this.attackHeight);
