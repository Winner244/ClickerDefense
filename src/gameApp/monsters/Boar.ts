	...
	logic(drawsDiffMs: number, buildings: Building[], monsters: Monster[], units: Unit[], bottomBorder: number, waveLevel: WaveData[]) {
		...
		if(this._isActivatedSpecialAbility) {
				...
				!this.isLeftSide && this.centerX <= this._specialAbilityXStart)
			{
				...
				AudioSystem.play(this.centerX, SoundStartSpecial, 0, 1, true, true).then(sourse => this._startSpecialSound = sourse);
				AudioSystem.play(this.centerX, SoundRunning, 0, 2, true, true, 1.2).then(sourse => this._runningSound = sourse);
			}
		}
