	...
	logic(millisecondsDifferent: number, buildings: Building[], bottomBorder: number) {
		...
		if(this.isActivatedSpecialAbility) {
				...
				Math.random() < 0.1 * millisecondsDifferent / 100)
			{
				...
				AudioSystem.play(this.centerX, SoundStartSpecial, 0.3, false, 1, true, null, true).then(res => this.startSpecialSound = res);
				AudioSystem.play(this.centerX, SoundRunning, 0.5, false, 2, true, null, true, 1.2).then(sourse => this.runningSound = sourse);
			}
		}
