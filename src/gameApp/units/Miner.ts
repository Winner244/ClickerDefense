...
export class Miner extends Unit{
	...
	public static readonly SPEED_PARAMETER = 'miner_speed_parameter';
	public static readonly DAMAGE_PARAMETER = 'miner_damage_parameter';
	public static readonly SELF_DEFENSE_IMPROVEMENT = 'self_defense_improvement';
	public static readonly WOOD_ARMOR_IMPROVEMENT = 'wood_armor_improvement';
	public static readonly GOLD_PICK_IMPROVEMENT = 'gold_pick_improvement';
	public static readonly DIAMOND_PICK_IMPROVEMENT = 'diamond_pick_improvement';
	...
	this.infoItems.splice(1, 0, new ParameterItem(Miner.SPEED_PARAMETER, 'Скорость', () => this.speed, speedIcon, 22, () => Miner.shopItem.price * 0.2, () => this.improveSpeed()));

		this.improvements.push(new Improvement(Miner.SELF_DEFENSE_IMPROVEMENT, 'Самооборона', Miner.shopItem.price * 2, shieldAndSwordsIcon, () => this.improveToSelfDefense(), [
			new ImprovementParameterItem(`+`, swordIcon)
		]));

		this.improvements.push(new Improvement(Miner.WOOD_ARMOR_IMPROVEMENT, 'Деревянная броня', Miner.shopItem.price * 2, WoodArmorImage, () => this.improveToWoodArmor(), [
			new ImprovementParameterItem(`+`, shieldIcon)
		]));

		this.improvements.push(new Improvement(Miner.GOLD_PICK_IMPROVEMENT, 'Золотая кирка', Miner.shopItem.price * 0.8, PickGoldImage, () => this.improveToGoldPick(), [
			new ImprovementParameterItem(`x2`, coinIcon)
		]));

		this.improvements.push(new Improvement(Miner.DIAMOND_PICK_IMPROVEMENT, 'Алмазная кирка', Miner.shopItem.price * 0.8, PickDiamondImage, () => this.improveToDiamondPick(), [
	...
	improveToGoldPick(){
		...
		let t = this.improvements.find(x => x.id == Miner.GOLD_PICK_IMPROVEMENT);
	...
	improveToDiamondPick(){
		...
		let t = this.improvements.find(x => x.id == Miner.DIAMOND_PICK_IMPROVEMENT);
		if(t) t.isImproved = true;
	}
	...
	improveToWoodArmor(){
		...
		let t = this.improvements.find(x => x.id == Miner.WOOD_ARMOR_IMPROVEMENT);
		if(t) t.isImproved = true;
	}

	improveToSelfDefense(){
		this.damage = 0.1;
	this.infoItems.splice(2, 0, new ParameterItem(Miner.DAMAGE_PARAMETER, 'Урон', () => this.damage.toFixed(1), swordIcon, 13, () => Miner.shopItem.price, () => this.damage += 0.1));
	
		let t = this.improvements.find(x => x.id == Miner.SELF_DEFENSE_IMPROVEMENT);
		if(t) t.isImproved = true;
	}
