...
export class Barricade extends Building{
	...
	public static readonly IRON_SPIKES_IMPROVEMENT = 'iron_spikes_improvement';

	public static readonly DAMAGE_RETURN_PARAMETER = 'damage_return_parameter';
	...
	loadedResourcesAfterBuild(){
		...
		this.infoItems.splice(1, 0, new ParameterItem(Barricade.DAMAGE_RETURN_PARAMETER, 'Возврат урона', () => this.damageMirrorPercentage + '%', SwordIcon));

		this.improvements.push(new Improvement(Barricade.IRON_SPIKES_IMPROVEMENT, 'Железные шипы', 100, BarricadeIronImage, () => this.impoveToIron(), [
	...
	impoveToIron(){
		...
		let t = this.improvements.find(x => x.id == Barricade.IRON_SPIKES_IMPROVEMENT);
